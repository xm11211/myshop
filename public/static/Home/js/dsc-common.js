...
$(function(){
	...
	$("*[ectype='cateItem']").on('mouseenter',function(){
		...
		if(eveval != 1){
			T.data('eveval', '1');
			/*加载中by wu*/
			layer.find("*[ectype='subitems_" + cat_id + "']").html('<img src="/static/home/img/loadGoods.gif" width="200" height="200" class="lazy">');
			$.ajax({
			   type: "GET",
			   url: getGoodsCates,
	...
	doc.on('click',"*[ectype='changeBrand']",function(){
		$.ajax({
			...
			data:{page:brandPage},
			success:function (data) {
                $("#recommend_brands").html(data.content);
				if(brandPage == data.totalPage) {
                    brandPage = 1;
                    return;
				}
				brandPage ++;
	...
	$("#go_pay").click(function(){
		//未登录的情况下，点击去支付之前先判断是否勾选商品，然后再显示登录弹窗
        if(!get_toCart()) {
            return false;
		}
        var back_url=$(this).data("url");
        $.notLogin(getLoginDialog,back_url);
        return false;
	});
