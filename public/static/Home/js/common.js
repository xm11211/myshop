...
function addToCart(goodsId,flag)
{
    ...
    // }
    //商品单选属性
    var checkedRadios = $(".check-status").find("input");
    var goodsAttr = [];
    if(checkedRadios.length > 0) {
        checkedRadios.each(function (k,v) {
            goodsAttr.push(v.value);
        });
    }
    goodsAttr = goodsAttr.toString();
    var goods = {goodsAttr: goodsAttr,goodsId: goodsId,number: 1};
    if(flag) {
        Ajax.call(ajaxAddToCart, 'goods=' + $.toJSON(goods), addToCartShowDivResponse, 'POST', 'JSON');
    }else {
        Ajax.call(ajaxAddToCart, 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
    }
...
function myAddToCartResponse(result) {
    if(result.error == 0) {
        alert(result.cartNum);
        $("em.cart_num").html(result.cartNum);
    }
}
var client_x=0,client_y=0,img_url=0,divId="",store_id=0,end_time='',store_mobile='';
function addToCart1(goodsId, parentId,event,obj,divId,store_id,end_time,store_mobile)
{
